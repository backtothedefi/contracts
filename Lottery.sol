...
// 4 numbers
contract Lottery is BEP20('BackToTheDefi Lottery Pool Token', 'BLPT') {
    ...
    // Price Fee address // Plan is to burn it.
    address public priceFeeAddress;
    // Price fee in basis points initially set to 20%
    uint16 priceFeeBP = 2000;
    // Max Price fee: 25%.
    uint16 public constant MAXIMUM_PRICE_FEE_BP = 2500;
    // Tx Fee: 7.5%
    uint txFee = 75;
    // =================================
    ...
    function injectIntoInitialPot(uint256 _price) internal {
        ...
        tndr.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external inDrawingPhase {
        require (_price >= minPrice, 'price must above minPrice');
        _price = _price.mul(txFee).div(100);
        ...
        tndr.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward>0) {
            ...
            tndr.safeTransfer(address(priceFeeAddress), priceFee);
            //Send to user
            tndr.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            ...
            tndr.safeTransfer(address(priceFeeAddress), priceFee);
            //Send to user
            tndr.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) external onlyOwner {
        tndr.safeTransfer(address(msg.sender), _amount);
    ...
    function updateTxFee(uint _txFee) external onlyOwner {
        require(_txFee <=100);
        txFee = _txFee;
    }
